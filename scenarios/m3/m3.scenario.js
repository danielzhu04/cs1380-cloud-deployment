...
*/
  ...
  distribution.local.groups.put(config, groupB, (e, v) => { // used to use 'initialNodes' instead of 'groupB,' changed per Ed post #142
    // Add a new node dynamically to the group
    groupB[id.getSID(n3)] = n3;

    distribution.local.groups.add('groupB', n3, (e, v) => {
      distribution.groupB.groups.put(config, groupB, (e, v) => { // used to use 'allNodes' instead of 'groupB,' changed per Ed post #142
...
*/
  ...
  distribution.local.groups.put(config, groupC, (e, v) => {
    distribution.groupC.groups.put(config, groupC, (e, v) => {
      ...
      const remote = {node: n2, service: 'groups', method: 'rem'};
...
*/
  ...
  const groupD = {}; // num elements significantly larger than lst choice seems to lead to higher likelihood of test failure
  ...
  let nExpected = 2; // larger numbers seem to lead to higher likelihood of test failure

  // Experiment with the subset function used in the gossip service...
  let config = {gid: 'groupD', subset: (lst) => 2}; // larger lst values seem to lead to lower likelihood of test failure
    ...
    distribution.groupD.groups.put(config, groupD, (e, v) => {
      ...
      distribution.groupD.groups.put('newgroup', {}, (e, v) => {
        ...
        distribution.groupD.gossip.send(message, remote, (e, v) => {
          ...
          let delay = 10; // larger delay seems to lead to higher counts
          setTimeout(() => {
